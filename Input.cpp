...
int Input::GetInteger(Output *pO) const 
{
	...
	string intused=GetSrting(pO);
	int number = stoi(intused);
...
{	
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		...
		{	
			...
			case ITM_copy: return copycard;
			case ITM_cut: return cut;
			case ITM_paste:return paste;
			case ITM_editcard: return editcard;
			case ITM_deletegame: return deletegame;
			case ITM_save: return savegrid;
			case ITM_opengrid: return opengrid;
...
CellPosition Input::GetCellClicked() const
{
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		if ( y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			...
			cellPos.SetHCell(10 - (UI.width - x) / 110);
			cellPos.SetVCell(8-(UI.height-UI.ToolBarHeight-y)/55);
			//done
