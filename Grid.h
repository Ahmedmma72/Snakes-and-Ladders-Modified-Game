...
public:
	...
	void SetClipboard(Card * card, bool deleteObjcet=0); //bool for cut // A setter to be used in copy/cut (in order NOT to break class responsibilities)
	Card * GetClipboard(bool &deleteObjcet);	 // A getter to be used in paste (in order NOT to break class responsibilities)


	Card* GetGetCardFromPos(CellPosition* pos);
	...
	void setCurrentPlayer(int p); //for new game
	...
	Card* hascard(CellPosition pos);                                 //       and the cards/snakes/ladders positions do NOT change already in Play Mode

	void PrintErrorMessage(string msg); // Prints an error message on statusbar, Waits for mouse click then clears statusbar
	void cleargrid();							    // We added this function once here because it is used many times by other classes
