...
void Output::DrawCardNumber(const CellPosition& cellPos, int cardNum) const
{
	...
	// 1 indicates that there already exists a card
...
//void Output::basicdrawcell(const CellPosition& cellPos, int cardNum) const
//{
//
//
//}
void Output::completeladdertop(const CellPosition& fromCell)   const
{
	...
	int y2 = fromStartY + UI.CellHeight;
	...
	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
	pWind->DrawLine(x12, y2, x34, y2);


}
void Output::completeladderbody(const CellPosition& fromCell)   const
{
	...
	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
	pWind->DrawLine(x12, y1, x34, y1);
	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
	pWind->DrawLine(x12, y2, x34, y2);

}
void  Output::completesnaketail(const CellPosition& fromCell)   const
{
	...
	int y1 = fromStartY;
	int y2 = fromStartY + UI.CellHeight / 2;
	pWind->SetPen(UI.SnakeColor, UI.SnakelineWidth);
	pWind->DrawLine(x12, y1, x12, y2);

}
void  Output::completesnakebody(const CellPosition& fromCell)   const
{
	int cellStartX = GetCellStartX(fromCell);
	int fromStartY = GetCellStartY(fromCell);
	int x12 = cellStartX + UI.LadderXOffset / 2;
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_editcard] = "images\\editcard.jpg";
	MenuItemImages[ITM_deletegame] = "images\\deletegame.jpg";
	MenuItemImages[ITM_save] = "images\\save.jpg";
	MenuItemImages[ITM_opengrid] = "images\\opengrid.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_input] = "images\\input.jpg";
	MenuItemImages[ITM_newgame] = "images\\newgame.jpg";
	MenuItemImages[ITM_Exit] = "images\\deletegame.jpg";
	MenuItemImages[ITM_SWITCH_TO_DESIGN_MODE] = "images\\Menu_SwitchToGrid.jpg";

	///TODO: Prepare images for each menu item and add it to the list //not complete same problem
...
void Output::PrintPlayersInfo(string info)
{
	...
	ClearToolBar();
	CreatePlayModeToolBar();//ClearToolBar();
	...
	pWind->GetStringSize(w, h, info);  //done
	...
	pWind->DrawString(x, y, info);   //done
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	pWind->GetIntegerSize(w, h, cellNum); //done
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	{
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
		int yi = fromStartY + i * UI.CellHeight;

		pWind->DrawLine(x12, yi, x34, yi);
	}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int xarr[4] = { x12 - xChange,x12,x12 + xChange ,x12 };
	int yarr[4] = { y1 - yChange,y1,y1 - yChange,y1 - 2 * yChange };
	...
	//filling shape and card indices
